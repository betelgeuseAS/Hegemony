...
export const FileUpload = ({ files, onSetFiles }) => {
  ...
  return (
    <>
      <FilePond
        ...
        //iconUndo={'<svg></svg>'}
        // Callbacks
        oninit={() => { console.log("FilePond instance has initialised", /*this.pond*/inputElement); }}//()
        ...
        onupdatefiles={fileItems => {//(files)
          ...
          onSetFiles(fileItems.map(fileItem => fileItem.file));
        }}
        onactivatefile//(file)
        // Hooks
        beforeDropFile//(file)
        ...
        //
        ref={ref => (/*this.pond = ref*/inputElement.current = ref)}
